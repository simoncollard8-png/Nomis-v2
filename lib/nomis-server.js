...
export async function saveMessage(role, content, parsedData = null) {
  try {
    await getSupabase().from('chat_messages').insert({ role, content, parsed_data: parsedData })
...
export async function loadUserContext() {
  ...
  const context = []
  try {
    const { data: workouts } = await db.from('workouts').select('*').order('date', { ascending: false }).limit(5)
    if (workouts?.length) {
    ...
    const { data: stats } = await db.from('body_stats').select('*').order('date', { ascending: false }).limit(1)
    if (stats?.length) {
      const s = stats[0]
      context.push(`CURRENT STATS: ${s.weight_lbs}lbs${s.body_fat_pct ? `, ${s.body_fat_pct}% BF` : ''} (${s.date})`)
    }

    const { data: sleep } = await db.from('sleep').select('*').order('date', { ascending: false }).limit(3)
    if (sleep?.length) {
    ...
    const { data: peptides } = await db.from('peptide_cycles').select('*').eq('status', 'active').limit(1)
    if (peptides?.length) {
      const p = peptides[0]
    ...
    const { data: meals } = await db.from('nutrition').select('*').eq('date', today)
    if (meals?.length) {
