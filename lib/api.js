...
export async function getRecentWorkouts(limit = 14) {
...
export async function logCardio(data) {
  return await dbWrite('cardio', 'insert', {
    ...
    pace_min_mile: parseFloat(data.pace_min_mile) || null,
    avg_heart_rate: parseInt(data.avg_heart_rate) || null,
    max_heart_rate: parseInt(data.max_heart_rate) || null,
...
export async function logNutrition(data) {
  return await dbWrite('nutrition', 'insert', {
    ...
    potassium_mg: parseInt(data.potassium_mg) || null,
    notes: data.notes || null
  })
}
...
export async function getTodaysSummary() {
  ...
  return { workouts, sleep, cardio, bodyStats, date: today }
}
