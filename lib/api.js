...
export async function getExerciseInfo(exercise_name) {
  ...
  if (info) {
    await setCache(cacheKey, 'exercise_info', info)
  }
...
export async function suggestExercises(muscle_group, count = 5, context = '') {
  ...
  if (isBaseRequest && suggestions.length) {
    await setCache(cacheKey, 'exercise_suggest', suggestions)
  }
...
export async function getSwapOptions(exercise_name, context = '') {
  ...
  if (options.length) {
    await setCache(cacheKey, 'exercise_swap', options)
  }
...
export async function getTodaysWorkout() {
  ...
  try {
    ...
    console.warn('user_schedule read failed, falling back to programs:', e)
  }
